...
function randomID(length = 10, type = ["letter", "number"]) {
    if (typeof (type) === "string" && (type !== "letter" && type !== "number" && type !== "symbol"))
    ...
    Array.isArray(type) && type.forEach((type, i) => { if (type !== "letter" && type !== "number" && type !== "symbol")
        throw new Error(`Array of type should conatiner string equal to either 'number' or 'letter', 'symbol' but we got ${JSON.stringify(type)} at index : ${i}`); });
    let id = "", data = "";
